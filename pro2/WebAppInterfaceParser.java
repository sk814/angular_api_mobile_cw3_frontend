...
public class WebAppInterfaceParser {
	...
	static String[] nodes = { "abstract_method", "access_level", "arguments", "throws", "return", "exception", "parameter" };
		...
		case Node.ELEMENT_NODE: {
			if (Arrays.asList(nodes).contains(node.getNodeName())) {
				...
				if (node.getNodeName() == "parameter") {
					...
					spaceses++;
					System.out.print("" + spaces(spaceses)+ openCurly+ "\n" + spaces(spaceses) + quote + "type" + quote + ": " + quote
							+ node.getAttributes().getNamedItem("type").getNodeValue() + quote + "," + "\n");
